...
function setProp(element: Element, name: string, value: unknown) {
  if (name === 'className') {
...
function removeProp(element: Element, name: string, value: unknown) {
  if (name === 'className') {
    element.removeAttribute('class');
  } else if (name.startsWith('on') && typeof value === 'function') {
    element.removeEventListener(name.slice(2).toLowerCase(), value as EventListener);
  } else {
...
export function patch(
  ...
  if (oldVNode == null) {
    const newNode = render(newVNode as VNode);
  ...
  updateProps(existingNode as Element, oldVNode.props, newVNode.props);
