...
export const deepClone2Problem: PracticeProblem = {
  ...
  title: 'Deep Clone II',
  difficulty: 'Hard',
  description: 'Implement an advanced `deepClone(value)` function that supports common JavaScript built-ins and circular references.\n\nIn addition to primitives, arrays, and plain objects, support `Date`, `RegExp`, `Map`, and `Set`. If the input graph contains cycles or shared references, the clone should preserve that structure without infinite recursion.',
  examples: [
    {
      input: '`const obj = { name: "Ada" }; obj.self = obj;`\n`const cloned = deepClone(obj);`\n`cloned !== obj` is `true`, and `cloned.self === cloned` is also `true`.',
      output: 'The clone is a new object and its circular `self` reference points to the clone',
    },
    {
      input: '`const map = new Map([[{ id: 1 }, new Set([new Date(0)])]]);`\n`deepClone(map)`',
      output: 'a new `Map` whose key, set, and date are all cloned values',
    }
  ],
  points: [
