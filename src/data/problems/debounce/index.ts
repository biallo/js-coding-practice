...
export const debounceProblem: PracticeProblem = {
  ...
  examples: [
    {
      input: 'Call `debounced("a")`, then call `debounced("b")` again before `wait` expires.',
      output: '`func` runs once after `wait` with `"b"`',
    },
    {
      input: 'Call `debounced.call(context, 1, 2)` and make no more calls before `wait` expires.',
