...
export const sleepProblem: PracticeProblem = {
  ...
  title: 'Sleep',
  difficulty: 'Easy',
  description: 'Given a positive integer `millis`, write an asynchronous function `sleep(millis)` that waits for `millis` milliseconds before resolving.\n\nThe returned promise can resolve with any value. A small deviation from the requested duration is acceptable because JavaScript timers are scheduled by the runtime.',
  examples: [
    {
      input: '`let start = Date.now();`\n`await sleep(100);`\n`Date.now() - start` should be approximately `100`.',
      output: 'The elapsed time is approximately `100ms`',
    },
    {
      input: '`await sleep(200)` should resolve after approximately `200` milliseconds.',
      output: 'The returned promise resolves after approximately `200ms`',
    }
  ],
  points: [
