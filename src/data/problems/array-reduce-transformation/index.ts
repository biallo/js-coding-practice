...
export const arrayReduceTransformationProblem: PracticeProblem = {
  ...
  difficulty: 'Easy',
  description:
    'Given an integer array `nums`, a reducer function `fn`, and an initial value `init`, return the final result obtained by applying `fn` to each element in order.\n\nThe reduction starts with `init`: `val = fn(init, nums[0])`, then `val = fn(val, nums[1])`, and so on until every element has been processed. Return the final value.\n\nIf `nums` is empty, return `init`.\n\nPlease solve it without using the built-in `Array.reduce` method.\n\nExamples:\n`reduce([1, 2, 3, 4], (accum, curr) => accum + curr, 0)` returns `10`.\n`reduce([1, 2, 3, 4], (accum, curr) => accum + curr * curr, 100)` returns `130`.\n`reduce([], () => 0, 25)` returns `25`.',
  points: [
