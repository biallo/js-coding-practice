...
export const squashObjectProblem: PracticeProblem = {
  ...
  title: 'Squash Object',
  difficulty: 'Medium',
  description: 'Implement `squashObject(object)`, which returns a new object after flattening a nested object into a single level of depth. Nested keys should be joined together with a period delimiter (`.`).\n\nArrays should also be traversed, with array indexes becoming path segments. Primitive values, `null`, and `undefined` should be assigned to the flattened path. Empty string keys are special: when joining paths, do not add an extra period for an empty segment.',
  examples: [
    {
      input: '`squashObject({ a: 5, b: 6, c: { f: 9, g: { m: 17, n: 3 } } })`',
      output: '`{ a: 5, b: 6, \'c.f\': 9, \'c.g.m\': 17, \'c.g.n\': 3 }`',
    },
    {
      input: '`squashObject({ a: { b: null, c: undefined } })`',
      output: '`{ \'a.b\': null, \'a.c\': undefined }`',
    },
    {
      input: '`squashObject({ a: { b: [1, 2, 3], c: [\'foo\'] } })`',
      output: '`{ \'a.b.0\': 1, \'a.b.1\': 2, \'a.b.2\': 3, \'a.c.0\': \'foo\' }`',
    },
    {
      input: '`squashObject({ foo: { \'\': { \'\': 1, bar: 2 } } })`',
      output: '`{ foo: 1, \'foo.bar\': 2 }`',
    }
  ],
  points: [
